...
import { sendMessageToDialogflow } from "../api";
...
function Chatbot({ selectedChat, updateChatHistory, chatHistory = [] }) {
  ...
  async function submitNewMessage() {
    ...
    setMessages((draft) => [...draft, userMessage, { role: "assistant", content: "Typing...", loading: true }]);
    setNewMessage("");
    setIsLoading(true);

    try {
      const reply = await sendMessageToDialogflow(trimmedMessage);
  ...
  return (
    <div className="relative flex flex-col h-screen w-full bg-white font-urbanist">
      {/* Left Animated Image */}
      <div className="absolute left-6 top-1/2 transform -translate-y-1/2 opacity-90 transition-transform duration-700 ease-in-out">
        <img src={leftImages[imageIndex]} alt="Left Avatar" className="w-72 h-96 object-contain drop-shadow-lg" />
      </div>

      {/* Right Animated Image */}
      <div className="absolute right-10 top-[53%] transform -translate-y-1/2 opacity-90 transition-transform duration-700 ease-in-out">
        <img src={rightImages[imageIndex]} alt="Right Avatar" className="w-64 h-64 object-contain drop-shadow-lg" />
      </div>
