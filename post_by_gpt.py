...
async def chat(session_id: str, conversation_id: str, user_input: str = Body(..., embed=True)):
    ...
    try:
        ...
        if not isinstance(conversation_history, list):
            conversation_history = []  # Ensure it is always a list
        ...
        # 🔹 Update Firestore with both user and bot messages
        try:
            # Store the complete conversation history, including both user and bot messages
            conversation_ref.set({"messages": conversation_history}, merge=True)
